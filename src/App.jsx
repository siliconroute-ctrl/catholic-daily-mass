...
export default function App() {
  ...
  const stopAll = useCallback(() => {
    ...
    if (reflectionAudioRef.current) reflectionAudioRef.current.pause();
    setPlayState("idle");
    setReading(null);
    setReflectionPlaying(false);
  }, []);
  ...
  // check whether HD audio and a reflection exist for this date.
  useEffect(() => {
    stopAll();
    setHdAudio(null);
    setReflection(null);
  ...
  const onOpenReflection = () => {
    // Pause (not stop/reset) whatever's currently playing, so it resumes
    // from the same spot afterward instead of restarting from the bell.
    if (hdAudio && audioRef.current && !audioRef.current.paused) {
      audioRef.current.pause();
      setPlayState("paused");
    } else if (!hdAudio && playState === "playing") {
      tts.pause();
      setPlayState("paused");
    }
    setShowReflection(true);
    if (reflection?.hasAudio && reflectionAudioRef.current) {
      reflectionAudioRef.current.currentTime = 0;
      reflectionAudioRef.current
        .play()
        .then(() => setReflectionPlaying(true))
        .catch(() => setReflectionPlaying(false));
    }
  };

  const onToggleReflectionAudio = () => {
    const a = reflectionAudioRef.current;
    if (!a) return;
    if (reflectionPlaying) {
      a.pause();
      setReflectionPlaying(false);
    } else {
      a.play()
  ...
  const onHideReflection = () => {
    reflectionAudioRef.current?.pause();
    setReflectionPlaying(false);
    setShowReflection(false);
  };
              ...
              <>
                ...
                {reflection && (
                  <div className="reflection-block">
                    {reflection.hasAudio && (
                      <audio
                        ref={reflectionAudioRef}
                        src={`/reflections/${isoDateKey(date)}.mp3`}
                        preload="none"
                        onEnded={() => setReflectionPlaying(false)}
                        onPause={() => setReflectionPlaying(false)}
                        onPlay={() => setReflectionPlaying(true)}
                      />
                    )}
                    ...
                    {!showReflection ? (
                      ...
                      <div className="reflection-panel">
                        <h3 className="reflection-heading">A Quiet Reflection</h3>
                        <p className="reflection-text">{reflection.text}</p>
                        {reflection.hasAudio && (
                          <button className="reflection-listen" onClick={onToggleReflectionAudio}>
                            {reflectionPlaying ? "\u25A0 Stop" : "\u25B6 Listen to this reflection"}
                          </button>
                        )}
                        <p className="reflection-disclaimer">
                          This reflection is generated by AI for personal
                          prayer and reflection. It is not official Church
                          teaching, a homily, or a substitute for one.
                        </p>
                        <button className="reflection-toggle" onClick={onHideReflection}>
                          Hide reflection
                        </button>
                      </div>
                    )}
